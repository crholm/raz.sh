...
If you are not writing thins in Go or want to structure your app abit more. Putting an ingress proxy in front of your app, that handles and strip tls, might be a good idea. Some known proxies ones are `Nginx`, `Caddy` and `Traefik`. These can also handle things like load balancing, caching, rate limiting and so on.
